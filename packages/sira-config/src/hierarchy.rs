...
pub fn resolve_configuration(
    ...
    diagnostics.record(
        "hal.vram_limit_mb",
        &config.hal.vram_limit_mb.to_string(),
        ConfigOriginLayer::BuiltinDefault,
        "SiraConfig::default()",
    );
    diagnostics.record(
        "render.max_threads",
        &config.render.max_threads.to_string(),
        ConfigOriginLayer::BuiltinDefault,
        "SiraConfig::default()",
    );
    ...
    if let Some(val) = env_vars.get("hal.vram_limit_mb") {
        if let Ok(num) = val.parse::<usize>() {
            ...
            diagnostics.record(
                "hal.vram_limit_mb",
                val,
                ConfigOriginLayer::EnvironmentVariable,
                "SIRA_HAL_VRAM_LIMIT_MB",
            );
        }
    }
    ...
    if let Some(val) = cli_args.get("vram-limit") {
        if let Ok(num) = val.parse::<usize>() {
            ...
            diagnostics.record(
                "hal.vram_limit_mb",
                val,
                ConfigOriginLayer::CommandLineArgument,
                "--vram-limit",
            );
        }
    }
